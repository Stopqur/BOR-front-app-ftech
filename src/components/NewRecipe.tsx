...
const NewRecipe = ({ show, close }: INewRecipeProps) => {
  ...
  const dataToRecipe = async () => {
    try {
      ...
      if (
        ...
        const recipeId: any = recipe.data.id;
        await updateRecipeImg({ recipeId, formData });
        dispatch(getRecipes());
        close();
      } else {
        setError(true);
  ...
  return (
    <Modal show={show} onHide={close} centered>
      ...
      <Modal.Body>
        {error ? <div className="alert alert-danger">Fill in all the fields!</div> : null}
        <Form>
          <FormControl
            className="mb-3"
            value={title}
          ...
          />
          <label className="mb-2" htmlFor="recipe-img">
            Add recipe image
          </label>
          <FormControl
            id="recipe-img"
      ...
      <Modal.Footer>
        ...
        <Button variant="outline-success" onClick={dataToRecipe}>
          Add
        </Button>
      </Modal.Footer>
    </Modal>
