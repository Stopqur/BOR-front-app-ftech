...
const RecipeItem = ({ item, flagIcon, token }: RecipeProps) => {
  ...
  const handleAddWishRecipe = async ({ recipeId, userId }: WishProps) => {
    try {
      dispatch(addWishRecipe(recipeId, userId));
    ...
    <ListGroup.Item className="recipe">
          ...
          {token ? (
            <div className="recipe__icon-btn">
              <Button
                ...
                onClick={() => handleAddWishRecipe({ recipeId, userId })}
              >
                {heartIcon ? (
                  <svg
      ...
      {item.user_id === userId ? (
        <div className="d-flex justify-content-end">
          <Button variant="danger" onClick={() => handleDeleteRecipe('recipeId', `${recipeId}`)}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
